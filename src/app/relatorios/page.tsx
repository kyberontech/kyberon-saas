'use client'
// src/app/relatorios/page.tsx — com responsividade mobile
...
const inputCls = `
  bg-ky-bg border border-ky-border rounded-lg px-3 py-2.5
  text-sm text-ky-text outline-none focus:border-ky-primary/50 w-full
`
...
export default function RelatoriosPage() {
  ...
  function handleSearch() {
    if (!card) return
    setChartData(generateMockData(card.value))
    setSearched(true)
  }
  ...
  function exportCSV() {
    ...
    a.href = url; a.download = `kyberon_${card.variableName.replace(/\s/g,'_')}.csv`
      ...
      <main className="flex-1 overflow-y-auto pt-[52px] md:pt-0">
        ...
        <div className="p-4 md:p-6 flex flex-col gap-4">
          ...
          <div className="bg-ky-panel border border-ky-border rounded-xl p-4 md:p-5">
            ...
            {/* Empilha tudo em coluna no mobile, grid no desktop */}
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={handleSearch} disabled={!selectedCard}
              ...
              <button onClick={exportCSV} disabled={!searched}
          ...
          {searched && chartData.length > 0 && (
            <div className="bg-ky-panel border border-ky-border rounded-xl overflow-hidden">
              <div className="px-4 md:px-5 py-3 border-b border-ky-border">
                <p className="font-head text-[10px] tracking-[3px] text-ky-muted uppercase">Dados Tabelados</p>
              </div>
